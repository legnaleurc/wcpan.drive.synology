...
from ..types import RawServerConfig, ServerConfig
...
def main() -> None:
    ...
    backfill_p = subparsers.add_parser(
        "backfill",
        help="Reconcile node metadata under PATH with Synology Drive API; enriches media dimensions when volume_map is configured",
    )
        ...
        try:
            stats = asyncio.run(
                async_api_backfill(
                    ...
                    volume_map=dict(vm) if vm else None,
                )
            )
