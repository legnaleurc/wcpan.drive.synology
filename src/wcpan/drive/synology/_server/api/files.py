...
from ..types import SynologyApiRef, SynologyPath
from .types import SynologyFileInfo, synology_file_info_from_api_dict
...
async def get_file_metadata_by_id(
    ...
    if not isinstance(raw, dict):
        return None
    return synology_file_info_from_api_dict(cast(dict[str, Any], raw))
...
async def get_file_metadata_by_path(
    ...
    """GET file metadata via its Synology path string (SYNO.SynologyDrive.Files get)."""
    url = f"{network.api_base}/files"
    async with network.fetch(
        "GET",
        url,
        params={"path": str(syno_path)},
    ) as response:
        data = await response.json()
...
async def list_folder_by_path(
    ...
    if not data.get("success", True):
        raise Exception(f"Failed to list path {str(syno_path)!r}: {data}")
...
async def create_folder(
    network: NetworkService,
    parent_ref: SynologyApiRef,
    name: str,
) -> SynologyFileInfo:
    """Create a new folder.

    parent_ref is either a ``SynologyPath`` or a ``SynologyIdRef``.
    """
    url = f"{network.api_base}/files"
    async with network.fetch(
        "POST",
    ...
    if not isinstance(raw_data, dict):
        raise Exception(f"Failed to create folder {name!r}: missing data in response")
    return synology_file_info_from_api_dict(cast(dict[str, Any], raw_data))
...
async def move_file(
    network: NetworkService,
    file_id: str,
    new_parent_ref: SynologyApiRef,
) -> None:
    """Move a file to a different parent folder (async, fire-and-forget).

    new_parent_ref is either a ``SynologyPath`` or a ``SynologyIdRef``.
    """
    ...
    if not data.get("success", True):
        raise Exception(f"Failed to move {file_id!r} to {new_parent_ref!r}: {data}")


async def delete_file(network: NetworkService, file_id: str) -> None:
